...
const parseMethod = (method: Sexp): Result<ProcExp> =>{
    if(isArray(method) && first(method)=='lambda'){
        const argsAndBody = rest(method);
        const args = first(argsAndBody);
        const body = rest(argsAndBody);
        return parseProcExp(args, body);
    }
...
export const parseClassExp = (fields: Sexp, methods: Sexp): Result<ClassExp> => {
    ...
    const namesOfFuncs = map(m => m[0], methods);
    const bodyResult = mapResult(bodyMethod => parseMethod(second(bodyMethod)),methods);
    const methodsResult = bind(bodyResult, (body: CExp[]) => makeOk(zipWith(makeBinding, namesOfFuncs, body)));
    return bind(methodsResult,(methods: Binding[]) => makeOk(makeClassExp(map(makeVarDecl,fields), methods)));
} 
...
const unparseClassExp = (c: ClassExp) : string => 
    `(class (${map((f:VarDecl)=>f.var,c.fields).join(" ")}) (${map(unparseBinding,c.methods).join(" ")}))`

const unparseBinding  = (b : Binding) : string =>
